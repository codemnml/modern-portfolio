export interface Project {
    name: string;
    description: string;
    link: string;
    imgUrl: string;
    technologies: string[];
    apis: string[];
}

const projects: Project[] = [
    {
        name: 'nicefinds',
        description: "An amazon affiliate-based website designed for efficiency. Features a simple to use, yet powerful admin panel. Ability to publish products from amazon to the main page with an associated affiliate link. Products can be published manually or with a direct product link and a click of a button, be posted automatically on the front page, with a complete description, a tweet sent out with a photo, and saved to a database. Front page features ability to sort by categories, tag, or brand name, and infinite scrolling.",
        link: 'http://www.nicefinds.net/',
        imgUrl: 'https://s24.postimg.org/qebts6yrp/logo_favicon.png',
        technologies: ['Angular', 'MongoDB', 'Express', 'Node.js', 'Bootstrap', 'HTML', 'CSS', 'Javascript'],
        apis: ['Amazon Product API', 'Twitter API', 'Bitly API', 'Webshot API']
    },
    {
        name: 'learndevanagari',
        description: 'A simple and fun app to learn Nepali (devanagari script). Features a simple user interface to test characters in study mode, account creation to save progress and stats, leaderboards to see how others compare, and a learn section to get an overview of the script. Also features an admin panel to add, edit, and/or remove characters for future revision or expansion.',
        link: 'https://learndev.herokuapp.com/',
        imgUrl: 'https://s9.postimg.org/topy52rm7/Screen_Shot_2017-03-10_at_1.01.31_PM.png',
        technologies: ['Angular', 'MongoDB', 'Express', 'Node.js', 'Bootstrap', 'HTML', 'CSS', 'Javascript'],
        apis: ['Gravatar API', 'Twitter API']
    },
    {
        name: 'ngportfolio',
        description: 'This current-facing website. Features a user-experience similar to that of a native iPhone or Android app. Features Angular Animations for smooth transitions between different routes.',
        link: 'https://ng-portfolio-nine-ashen.vercel.app/projects',
        imgUrl: 'https://s17.postimg.org/6ytcmy4zj/Copy_of_logo_favicon_ngportfolio.png',
        technologies: ['Angular', 'Angular Animations', 'Express', 'Node.js', 'Bootstrap', 'HTML', 'CSS', 'Javascript'],
        apis: ['Mailgun API']
    },
    {
        name: 'indeed-me',
        description: "A side-project to quickly get relevant data in a tabular format from indeed's website.",
        link: 'https://github.com/micustudio/indeed-me',
        imgUrl: 'https://s1.postimg.org/f29uzcdb3/ngportfolio_logo_favicon_ngportfolio.png',
        technologies: ['Angular', 'Express', 'Node.js', 'Bootstrap', 'HTML', 'CSS', 'Javascript'],
        apis: ['Indeed Api']
    },
    {
        name: 'travel-site',
        description: 'A flexible and customizable single page (brochure) site built with modern developer skills, tools, and best practices.',
        link: 'https://micustudio.github.io/travel-site/#',
        imgUrl: 'https://s2.postimg.org/rznwg2czd/Screen_Shot_2017-08-08_at_1.32.15_PM.png',
        technologies: ['HTML', 'CSS', 'Javascript', 'jQuery', 'Gulp', 'PostCSS', 'BEM Philosophy', 'Webpack'],
        apis: []
    }
];

export default projects;
